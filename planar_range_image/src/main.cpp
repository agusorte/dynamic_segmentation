...
//      
   ...
   ///planar range image
    ...
    AngleAxisf angle_axis(R_);
    Eigen::Vector3f eulerAngles = R_.eulerAngles(2, 0, 2);
    pose=pose*Translation3f(T_)*AngleAxisf(R_);
   // pose=pose*trans;
  //   pcl::transformPointCloud (*cloud_filtered, *cloud_filtered, trans);
    //pose=pose*AngleAxisf(R_);
    //pose=pose*angle_axis;

  
   // pose=pose*AngleAxisf(Trans_);
    
    cout<<"Translation1 " <<pose.translation()<<endl;
    cout<<"Rotation1 " <<pose.rotation()<<endl;
    cout<<"afinne3f" <<pose.matrix()<<endl;
    /*
    cout<<"Translation2 " <<pose3.translation()<<endl;
    cout<<"Rotation2 " <<pose3.rotation()<<endl;
    cout<<"afinne3f 2" <<pose3.matrix()<<endl;*/
   // cout<<"angle axis Rotation " <<angle_axis.matrix()<<endl;
     
    
   //Eigen::Affine3f pose = Eigen::Translation3f(T_) * Eigen::AngleAxisf(R_);
   //pose=pose*Eigen::AngleAxisf(Trans_);
   range_image_planar->createFromPointCloudWithFixedSize(*cloud_filtered,cv_ptr->image.cols,
     cv_ptr->image.rows,K_(0,2),K_(1,2),K_(0,0),K_(1,1),sensorPose,pcl::RangeImage::CAMERA_FRAME, noiseLevel, minRange);
...
//cout<<"Pass here --------"<<endl;
   if(!firstFrame)
   {
    ...
    drawOptFlowMap(flow, cflow, 40, 1.5, Scalar(0, 255, 0));
...
//   imshow("Range Image jet", falseColorsMap);
//   imshow("Range norm", im_range_current_norm);
//      
//    waitKey(0);
